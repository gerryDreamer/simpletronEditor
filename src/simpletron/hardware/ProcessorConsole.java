...
public class ProcessorConsole {
     ...
     */
    public ProcessorConsole(File inputFile, File outputFile)
    {
    ...
    public void execute()
    {
        ...
        outputStream.writeToGUI("instruction counter: "+instructionCounter.getInstructionCounter()); 
        /**
       ...
       * in a branching construct:
       ...
       //fetch the instruction from memory
             //set the initial index to begin the fetch
        int startIndex = 0;//begins from the memory location 0
        ...
        //begin the fetch:
                ...
                try
                {
                 do              
                 {
                   ...
                   */
                  if(operationCodeRegister.getOperationCode() != branch.getConstant() && operationCodeRegister.getOperationCode()!= branchZero.getConstant() && operationCodeRegister.getOperationCode() != branchNeg.getConstant())
                  {
                      if(done == false)
                      {
     ...
     * @deprecated refer to:: {@link simpletron.hardware.ProcessorConsole#setCurrentWord(int)  }
     */
...
class ProcessorConsoleTest
{
    public static void main(String[] args) {
        File input = new com.dreamer.file.FileFunctions().getOpenFile();
        File output = new com.dreamer.file.FileFunctions().getSaveFile();
        ProcessorConsole p = new ProcessorConsole(input,output);
