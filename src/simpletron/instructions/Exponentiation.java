...
/**
 * This object performs Exponential operations within an SML array
 * @author gerry dreamer
 */
public class Exponentiation extends Instructions{
     /**
     * the current constant defining this object
     */
    private final int constant = 35;
     /**
    ...
    * @param opCode the current opCode
     ...
     */
    public Exponentiation(OperationCodeRegister opCode, OperandRegister operand,Memory memory,Accumulator accumulator, InstructionRegister instructionRegister, InstructionCounter instructionCounter)
    {
    ...
    public void executeInstruction()
    {
        ...
        {
         //find the exponential of the value from the location in memory specified in the operand and the accumulator
           try{
             int a = getAccumulator().getResult();
             int b = getMemory().getElement(getOperandRegister().getOperand());
             //setOutput(new com.dreamer.recursion.ExponentialCalculator().exponential(b, b));//calculate
             setOutput((int) Math.pow(a, b));
