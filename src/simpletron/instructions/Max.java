...
/**
 * This object finds the largest value between the value loaded in the accumulator, and a value in the location specified by the SML instruction operand
 * @see simpletron.hardware.Accumulator
 * @see simpletron.hardware.OperandRegister
 * @author gerry dreamer
 */
public class Max extends Instructions{
     /**
     * the current constant defining this object
     */
    private final int constant = 36;
     /**
     ...
     */
    public Max(OperationCodeRegister opCode, OperandRegister operand,Memory memory,Accumulator accumulator, InstructionRegister instructionRegister, InstructionCounter instructionCounter)
    {
    ...
    public void executeInstruction()
    {
        ...
        {
           //find the max of the value from the location in memory specified in the operand and the accumulator
           try{
             int a = getAccumulator().getResult();
             int b = getMemory().getElement(getOperandRegister().getOperand());
             setOutput(Math.max(a, b));
